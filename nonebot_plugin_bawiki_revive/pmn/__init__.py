...
from cookit.pw import make_real_path_router, screenshot_html
...
from nonebot_plugin_picmenu_next.markdown import build_default_prp_processor
...
from nonebot_plugin_picmenu_next.templates.jj_utils import (
...
AliasCompatModel = get_model_with_config(
    {
        "alias_generator": lambda x: f"bawiki_rev_pmn_{x}",
        **compat_model_config,
    },
)
...
PICMENU_NEXT_DEFAULT_RES_DIR = BUILTIN_TEMPLATE_DIR / "default" / "res"
debug = DebugFileWriter(Path.cwd() / "debug", "bawiki-revive", "picmenu")
template_render = get_template_render(
    None,
    frozenset({RenderBackend.PLAYWRIGHT}),
    "BAWiki Revive PicMenu template",
...
@routers.router(f"{ROUTE_BASE_URL}/markdown/**/*")
...
@routers.router(f"{ROUTE_BASE_URL}/picmenu-next/**/*")
@make_real_path_router
@log_router_err()
async def _(url: "URL", **_):
    return PICMENU_NEXT_DEFAULT_RES_DIR.joinpath(*url.parts[2:])


@routers.router(f"{ROUTE_BASE_URL}/**/*", 99)
@make_real_path_router
@log_router_err()
async def _(url: "URL", **_):
